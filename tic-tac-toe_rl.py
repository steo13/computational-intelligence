...
def main(train_epochs):
    ...
    for epochs in tqdm(range(train_epochs)):
        Q_table_next = dict(Q_table)            #Creation of the dictionary from Q_table
        for s, a in Q_table:
            sn = next_state(s, a)
            if sn in final_states:
                q = V_table[sn]
            else:
                next_moves = [
                    best_action(Q_table, (sn[0], frozenset(set(sn[1]) | {x})))
                    for x in valid_actions(sn, agent=1)
                ]
                if not valid_actions(sn, agent=1):
                    display(sn)
                    print(sn in final_states)
                q = sum(v for _, v in next_moves) / len(next_moves)
            Q_table_next[(s, a)] = V_table[s] + gamma * q
        Q_table = Q_table_next
    describe_policy(Q_table, V_table)

    board = (frozenset(), frozenset())
    '''
    display(([], []), coordinates=True)
    user = int(input("Move: "))
    board = (board[0], frozenset(board[1] | {user}))
    display(board, [f'User played {user}'])
    print()
    '''
    while board not in final_states:
        move, evaluation = best_action(Q_table, board)
        board = next_state(board, move)
        display(board, [f'Computer played {move}', f'Expected result {evaluation}'])
        print()
        if board not in final_states:
            display(([], []), coordinates=True)
            user = int(input("Move: "))
            print()
            board = (board[0], frozenset(board[1] | {user}))
            display(board, [f'User played {user}'])
            print()
    print(f'Final result: {eval_static(board)}')
